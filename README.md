...
#### Prompt 6 [Feature 20 closeout — widget target wiring, deep-link, polish nits, and Phase 4g test gaps] — 2026-04-20

- Widget Extension target is live. Split `WorkoutLiveActivityAttributes.swift` into a pure ActivityKit contract (shared with `SuggestMeSomeLiveActivityExtension` via a `PBXFileSystemSynchronizedBuildFileExceptionSet`) and a main-app-only `WorkoutLiveActivityAttributes+Session.swift` sibling that carries the `fromSession(...)` / `nextSetTarget(...)` helpers. `SuggestMeSomeLiveActivityBundle.swift` now hosts `WorkoutLiveActivityWidget()` under the iOS 16.1 availability gate; the Xcode template's `SuggestMeSomeLiveActivity.swift` / `SuggestMeSomeLiveActivityControl.swift` / `SuggestMeSomeLiveActivityLiveActivity.swift` are gone. `INFOPLIST_KEY_NSSupportsLiveActivities = YES` is set on both Debug and Release configurations of the main app target
- Live Activity taps deep-link to the running workout. Extended `AppDeepLinkRoute` with `.activeWorkout(sessionID:)` so `suggestmesome://workout/<uuid>` (the widget's `.widgetURL`) resolves through the existing `.onOpenURL { ... }` handler in `SuggestMeSomeApp.swift`. `ContentView` flips to the Workouts tab and, when the tapped sessionID still matches `ActiveWorkoutSessionStore.session.id`, opens the existing active-workout sheet — stale taps after a session has ended land on the Workouts tab with the banner dismissed instead of popping an empty sheet
- Polish nits from the Prompt 5 code review cleared:
  - `AsyncActionButton` tracks the running `Task` in view state and cancels it in `.onDisappear`, so a mid-flight network call doesn't leak past the view's lifetime or fire the success haptic into the void. Also dropped the explicit `await MainActor.run { ... }` hop since the Task inherits `@MainActor` from the enclosing view body
  - `NameEditorSheet` wraps `isFocused = true` in a 100ms `Task.sleep` so the focus state change lands after the sheet detent animation settles — keyboards now lift reliably on iPad and when Reduce Motion is off
  - `ExerciseDisplayFormatter` absorbs the last four inline helpers from `ProgramReviewView`: `workingSetStyleLabel` / `exercisePurposeLabel` / `exerciseSelectionReasonLabel` move into the Foundation-only file, `workingSetStyleColor` gets its own `ExerciseDisplayFormatter+SwiftUI.swift` sibling so the SwiftUI import doesn't pollute the headless formatter. Four new `Feature20UIModernizationTests` golden the extraction across cardio, all three strength styles, and tagged/untagged explainability codes
- Phase 4g test gaps from Feature 19 closed out as five new `Feature19CollaborationFoundationTests`:
  - `refreshErrorRecoversOnNextRefresh` — a one-shot `throwOnNextFetchRelationships` flag surfaces a `.refresh` endpoint error, and the follow-up refresh clears it and repopulates the cache
  - `accountSwitchClearsPriorAccountRelationships` — swapping `currentAccountID` from the seeded athlete account to a different coach account wipes the prior cache before loading the new data so no stale row leaks across the boundary
  - `roleFlipRepartitionsRelationshipBuckets` — same relationship seed, viewed as coach vs athlete via `currentAccountID` flip, re-partitions `coachRelationships` / `athleteRelationships` correctly
  - `pushTokenChurnDoesNotDoubleRegisterIdenticalTokens` — identical tokens delivered back-to-back register once, a different token registers a second time, and the rotated token also dedupes on repeat
  - `batchFiveHundredRelationshipColdLoadStaysUnderTwoSeconds` — a cold 500-relationship refresh stays well under the 2s budget (observed ~0.15s on iPhone 17 Pro simulator)
- Verification:
  - succeeded: `xcodebuild -project SuggestMeSome.xcodeproj -scheme SuggestMeSome -destination 'platform=iOS Simulator,name=iPhone 17 Pro' build` (widget extension `.appex` embeds into the main app and `embeddedBinaryValidationUtility` passes)
  - succeeded: `xcodebuild test -project SuggestMeSome.xcodeproj -scheme SuggestMeSome -destination 'platform=iOS Simulator,name=iPhone 17 Pro' -only-testing:SuggestMeSomeTests/Feature19CollaborationFoundationTests -only-testing:SuggestMeSomeTests/Feature20LiveActivityTests -only-testing:SuggestMeSomeTests/Feature20UIModernizationTests -only-testing:SuggestMeSomeTests/Feature20CollaborationStoresTests -only-testing:SuggestMeSomeTests/Feature20CloudClientCacheTests`
- Out of scope (deferred to follow-on features): device-only QA of the Live Activity layouts (Dynamic Island expanded / compact / minimal don't render on simulator), a full mid-refresh race test for the account switch (the coalescer's interaction with interleaved account changes would need a test double for the coalescer itself — the sequential-clear test lands the safety-net guarantee without that scaffolding)

#### Prompt 7 [Feature 20 deep audit — three bugs surfaced and fixed] — 2026-04-20

- Deep code review of every surface introduced in Prompts 1–6 (sub-stores, DS components, Live Activity contract / controller / widget / deep-link, GET coalescer + TTL cache, coordinator facade, view-lifecycle wiring). Focused on concurrency races, cache invalidation, and view-state cleanup — the categories most likely to hide bugs behind green tests. Three real bugs fixed:
  - `AsyncActionButton` could come back from a disappear/reappear cycle frozen as disabled-with-spinner. The cancelled branch of the in-flight Task bailed before flipping `isRunning` back off, and if `@State` survived (long scrolling list cells that recycle, sheet dismiss-then-reopen) the button rendered stuck until the containing view was fully re-created. `.onDisappear` now cancels the Task AND resets `isRunning = false`, so the next appearance is always clean
  - `CollaborationGETRequestCache.coalesce` cleared `inFlight[key]` from each caller's `await` context. If caller A was cancelled while piggybackers B and C were still waiting on the shared Task, A yanked the map entry out from under them and a subsequent caller D spawned a duplicate network request instead of joining. Cleanup now happens exactly once inside the shared Task itself via a private `finishInFlight(key:cachedData:)` actor method, keyed on the Task's own completion — cancellation of any caller (including the originator) no longer breaks coalescing for the remaining waiters
  - `ActiveWorkoutSessionStore.startSession` never reset `appliedWatchActionIDs`. In any flow where a new session starts without a `discardSession` in between (e.g. a save path that routes around discard), stale action IDs carried over to the new session's dedup set. UUID collision risk is zero in practice, but the semantics were wrong and a trap for future call sites. `startSession` now mirrors the reset that already existed in `discardSession`
- Noted but intentionally NOT fixed (cosmetic / not-a-bug, documented inline for future polish):
  - `DSToast` has a redundant `.accessibilityAddTraits(.isStaticText)` after `.accessibilityElement(children: .combine)` — `.combine` already achieves static-text behavior
  - `WorkoutLiveActivityController.update(...)` spawns a throwaway `Task` per session mutation — wasteful but ActivityKit coalesces in-flight updates downstream, so no correctness impact
  - `NameEditorSheet` `.onAppear` Task has no cancellation path — safe in practice because SwiftUI no-ops FocusState assignment on dismissed views, and the window is only 100ms
  - `Task.sleep(nanoseconds:)` is deprecated in favor of `Task.sleep(for:)` in iOS 18+ — warnings only
  - `CollaborationInsightsStore.coachRosterSnapshotsCache` reads `relationshipsStore.coachRelationships` through a closure; fragile by design but correct today because `loadCache()` always calls `.apply(...)` on every sub-store in lockstep
- Verification:
...
#### Prompt 8 [Post-feature audit + targeted fixes for the 9 findings] — 2026-05-02

- Ran a holistic audit across all 7 prior Feature 22 commits covering mass-migration safety, decomposition correctness, feature-flag end-to-end, Watch + Live Activity targets, new-component performance, scalability, accessibility, default-param-value semantics, and explicit builds of all 5 targets. Full serial test suite (295 tests) ran on a fresh simulator: 0 regressions introduced by F22; 2 pre-existing failures (`persistenceStoreSkipsWritesWhenEncodedPayloadIsUnchanged`, `healthKitCautionNudgesIntensityDown`) are unrelated to the UI work.
- Audit produced 9 findings ranked by severity. All 9 fixed in priority order with one commit each, no behavior changes outside the documented intent of each fix:
  - **F-2 (high)** — three `.indigo` literals were missed by P6's regex sweep because the loop was scoped to `Views/` and `Compliance/` only. Most impactful: `ContentView.swift:96` `.tint(.indigo)` was overriding the new violet AccentColor across the entire TabView. Migrated `ContentView.swift` (TabView tint, active workout banner background) and `Services/ExerciseDisplayFormatter+SwiftUI.swift` (working-set chip color) to `DSColor.primaryAction`. (commit `6eece9a`)
  - **F-1 (high)** — the `EnvironmentValues.uiRefreshV2` infrastructure introduced in P1 was dead code: nothing read `Environment(\.uiRefreshV2)`, so every `DSColor.primaryAction` access (≈80 reads per fresh render across DashboardView / DailyCoachView / DashboardTheme) was hitting `UserDefaults` directly. Added a thread-safe process-wide cache (`NSLock` + `nonisolated(unsafe)` static dict) on `FeatureFlag.isEnabled`: first read populates from UserDefaults, subsequent reads are O(1), the setter writes through to both UserDefaults and the cache. Added `resetCache()` for tests. (commit `4943934`)
  - **F-3 (medium)** — 24 plain `.font(.headline)` callsites that P6's mass migration loop missed because its directory list omitted `Compliance/`, `Workout/`, `ContentView`, and `DSToast`. Migrated all of them to `.dsHeadline()` so paywalls, compliance flows, and live-workout chrome read with the same rounded type design as the rest of the app. Chained patterns like `.font(.headline.weight(.bold))` are still left for per-callsite weight decisions. (commit `d37e31f`)
  - **F-5 (medium)** — `DSStat.swift` defined a `private struct DSMetricFont` that shadowed the canonical one in `DSTypography.swift` with different shape: the local `.medium` and `.small` cases lacked `.contentTransition(.numericText())`. Currently dead code (DSStat hardcodes `.large`) but a future-maintenance trap. Deleted the local shim and routed through `dsMetricLarge()` directly. (commit `05f8176`)
  - **F-4 (medium)** — `DSCard` was wrapping the optional header slot in `AnyView`, which type-erases the wrapped subtree and disables SwiftUI's identity-based diffing. Currently dormant (no callsite uses the header slot yet) but would have lit up as soon as a future surface adopted it. Made the type generic over `Header: View` with a designated init plus three `extension DSCard where Header == EmptyView` convenience inits so both existing call sites (`BlockContinuityCard`, `ExpandableCard`) compile unchanged. (commit `3c15dc8`)
  - **F-8 (low)** — `DSTextField`'s validation error landed silently for VoiceOver users: sighted users saw a red exclamation label, but the field's accessibility tree didn't update. Added `.accessibilityLabel(title)`, `.accessibilityValue(text or "Empty")`, and a `.accessibilityHint` that folds the helper text in normally and the error message (prefixed `"Error: "`) when present. Title and helper/error labels are now `accessibilityHidden(true)` so they aren't double-announced. (commit `ff21d22`)
  - **F-7 + F-9 (low)** — the `plainWorkoutRow` View extension was promoted from `fileprivate` to `internal` during P1 decomposition because callers in `WorkoutView.swift` needed access from a different file than the post-extraction location in `WorkoutSubviews.swift`. Moved it back to `WorkoutView.swift` and restored the `fileprivate` access modifier so target-wide visibility for the helper goes back to nil. F-9 (the `.tint(.indigo)` short-circuit in ContentView) was already fixed in F-2. (commit `19e6a9e`)
- Build verification: all 4 production targets (`SuggestMeSome`, `SuggestMeSomeWatch`, `SuggestMeSomeWatchWidget`, `SuggestMeSomeLiveActivityExtension`) build clean at HEAD.
- Verification:
  - succeeded: `xcodebuild build -scheme SuggestMeSome -destination 'platform=iOS Simulator,name=iPhone 17 Pro'`.
