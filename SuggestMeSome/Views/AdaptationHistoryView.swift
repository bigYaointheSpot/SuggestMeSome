    ...
    var body: some View {
        ...
        .toolbar {
            ToolbarItem(placement: .topBarTrailing) {
                Button(showDebugDetails ? "Hide Debug" : "Show Debug") {
        ...
        Section("Recent Weekly Analyses") {
            ForEach(Array(analyses.prefix(8))) { analysis in
                VStack(alignment: .leading, spacing: 6) {
                    ...
                    if showDebugDetails {
                        Text("signals=\(fmt1(analysis.totalSignalWeight)); adherence=\(fmtPercent(analysis.adherenceScore)); hardSets=\(fmt1(analysis.totalCompletedHardSets)); fatigueScore=\(fmt2(analysis.observedFatigueScore))")
                            .font(.caption2)
                            .foregroundStyle(.secondary)
                    }
                }
            ...
            if latestTrendByLift.isEmpty {
                ...
                ForEach(latestTrendByLift) { snapshot in
                    VStack(alignment: .leading, spacing: 6) {
                        ...
                        if showDebugDetails {
                            Text("week=\(snapshot.programWeekNumber.map(String.init) ?? "n/a"); dataPoints=\(snapshot.totalDataPoints); weightedSignals=\(fmt1(snapshot.weightedSignalCount)); topSet=\(topSetText(snapshot))")
                                .font(.caption2)
                                .foregroundStyle(.secondary)
                        }
                    }
            ...
            if proposals.isEmpty {
                ...
                ForEach(Array(proposals.prefix(16))) { proposal in
                    VStack(alignment: .leading, spacing: 6) {
                        ...
                        if showDebugDetails, let detail = proposal.detailText, !detail.isEmpty {
                            Text(detail)
                                .font(.caption2)
                                .foregroundStyle(.secondary)
                                .lineLimit(5)
            ...
            if overlays.isEmpty {
                ...
                ForEach(Array(overlays.prefix(16))) { overlay in
                    VStack(alignment: .leading, spacing: 6) {
                        ...
                        if showDebugDetails {
                            let compact = overlay.adjustments
                                .sorted { $0.sequence < $1.sequence }
                                .prefix(3)
                                .map { adjustment in
                                    "\(adjustment.adjustmentType.rawValue)[wk=\(adjustment.targetWeekNumber.map(String.init) ?? "all"), sess=\(adjustment.targetSessionNumber.map(String.init) ?? "all")]"
                                }
                                .joined(separator: "; ")
                            if !compact.isEmpty {
                                Text(compact)
                ...
                ForEach(Array(events.prefix(30))) { event in
                    VStack(alignment: .leading, spacing: 6) {
                        ...
                        if let explanation = event.explanation, !explanation.isEmpty {
                            Text(showDebugDetails ? explanation : userFacingExplanation(from: explanation))
                                .font(.caption2)
                                .foregroundStyle(.secondary)
                                .lineLimit(showDebugDetails ? 8 : 3)
                        }

                        if showDebugDetails {
                            let requiresUserAction = event.requiresUserAction ? "true" : "false"
                            Text("week=\(event.analysisWeekNumber.map(String.init) ?? "n/a"); lift=\(event.targetLiftKey ?? "n/a"); confidence=\(fmt2(event.confidenceSnapshot)); userActionRequired=\(requiresUserAction)")
                                .font(.caption2)
                                .foregroundStyle(.secondary)
                        }
                    }
