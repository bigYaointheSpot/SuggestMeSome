...
enum SessionOutcomeInferenceService {
    ...
    private static func inferProgramLinkedScore(
        ...
        let isPowerliftingLift = canonicalLiftKey == CanonicalLift.squat.rawValue || canonicalLiftKey == CanonicalLift.bench.rawValue || canonicalLiftKey == CanonicalLift.deadlift.rawValue
    ...
    private static func resolveCanonicalLiftKey(for exerciseName: String) -> String? {
        ...
        if normalized.contains("overhead press") || normalized.contains("strict press") { return CanonicalLift.overheadPress.rawValue }
